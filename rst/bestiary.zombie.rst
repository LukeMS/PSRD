...
.. _`bestiary.zombie#human_zombie`:
...
\ **DR**\  5/slashing; \ **Immune**\  :ref:`undead traits <bestiary.universalmonsterrules#undead_traits>`

.. _`bestiary.zombie#offense`:
...
Although capable of following orders, zombies are more often unleashed into an area with no command other than to kill living creatures. As a result, zombies are often encountered in packs, wandering around places the living frequent, looking for victims. Most zombies are created using :ref:`animate dead <corerulebook.spells.animatedead#animate_dead>`\ . Such zombies are always of the standard type, unless the creator also casts :ref:`haste <corerulebook.spells.haste#haste>`\  or :ref:`remove paralysis <corerulebook.spells.removeparalysis#remove_paralysis>`\  to create fast zombies, or :ref:`contagion <corerulebook.spells.contagion#contagion>`\  to create plague zombies.

.. _`bestiary.zombie#creating_a_zombie`:
...
\ **Attacks**\ : A zombie retains all the natural weapons, manufactured weapon attacks, and weapon proficiencies of the base creature. It also gains a slam attack that deals damage based on the zombie's size, but as if it were one size category larger than its actual size  (see :ref:`Natural Attacks <bestiary.universalmonsterrules#natural_attacks>`\ ).
...
\ **Feats**\ : A zombie loses all feats possessed by the base creature, and does not gain feats as its Hit Dice increase, but it does gain :ref:`Toughness <corerulebook.feats#toughness>`\  as a bonus feat.
...
.. _`bestiary.zombie#variant_zombies`:
...
.. _`bestiary.zombie#plague_zombie`:
