...
.. _`ultimatecombat.combat.performancecombat`:

.. contents:: \ 

.. _`ultimatecombat.combat.performancecombat#performance_combat`:
...
.. _`ultimatecombat.combat.performancecombat#types_of_performance_combat`:
...
.. _`ultimatecombat.combat.performancecombat#knockout_bout`:
...
.. _`ultimatecombat.combat.performancecombat#staged_combats:`:

\ **Staged Combats:**\  Staged combats are less dangerous than knockout bouts. In these contests, combatants typically arrange to have blows that just barely land, so the hits are not registered as either nonlethal or lethal damage, but crowd reaction is determined in the same way as for normal battles. Staged combats often require advanced training (see the :ref:`Stage Combatant <ultimatecombat.ultimatecombatfeats#stage_combatant_(combat)>`\  feat). Those without advanced training can attempt to participate, but take a –6 penalty on attack rolls in order to make it seem like their attacks hit without doing real damage.

.. _`ultimatecombat.combat.performancecombat#to_a_number_of_wounds`:
...
.. _`ultimatecombat.combat.performancecombat#to_the_death`:
...
.. _`ultimatecombat.combat.performancecombat#toward_a_goal`:
...
.. _`ultimatecombat.combat.performancecombat#crowd_attitudes`:
...
.. _`ultimatecombat.combat.performancecombat#hostile`:
...
.. _`ultimatecombat.combat.performancecombat#unfriendly`:
...
.. _`ultimatecombat.combat.performancecombat#indifferent`:
...
.. _`ultimatecombat.combat.performancecombat#friendly`:
...
.. _`ultimatecombat.combat.performancecombat#helpful`:
...
.. _`ultimatecombat.combat.performancecombat#crowds_starting_attitude`:
...
.. _`ultimatecombat.combat.performancecombat#performance_combat_check_dc`:
...
* \ :sup:`1`\ This is the DC to gain a victory point.
...
.. _`ultimatecombat.combat.performancecombat#size_of_the_crowd`:
...
.. _`ultimatecombat.combat.performancecombat#small_crowd`:
...
.. _`ultimatecombat.combat.performancecombat#medium_crowd`:
...
.. _`ultimatecombat.combat.performancecombat#large_crowd`:
...
.. _`ultimatecombat.combat.performancecombat#massive_mob`:
...
.. _`ultimatecombat.combat.performancecombat#total_number_of_combatants`:
...
.. _`ultimatecombat.combat.performancecombat#small_battle`:
...
.. _`ultimatecombat.combat.performancecombat#medium_battle`:
...
.. _`ultimatecombat.combat.performancecombat#large_battle`:

\ *Large Battle*\ : A battle with 17 or more total combatants increases the DC to improve the attitude of the crowd by +8.

.. _`ultimatecombat.combat.performancecombat#other_circumstances_that_affect_dc`:
...
.. _`ultimatecombat.combat.performancecombat#cheating`:

\ *Cheating*\ : If one side visibly cheats or breaks the rules, the DC for that side of the performance combat to improve the attitude of the crowd increases by 2 for the remainder of the performance combat. This increase can occur multiple times in a battle. For the purpose of spotting cheating, determine the base :ref:`Perception <corerulebook.skills.perception#perception>`\  and :ref:`Sense Motive <corerulebook.skills.sensemotive#sense_motive>`\  for the average member of a crowd (usually +0 if you assume an average human) and apply the following modifiers based on crowd size. The DC is based on whatever action the combatant is using to hide any cheating (usually :ref:`Bluff <corerulebook.skills.bluff#bluff>`\  or :ref:`Stealth <corerulebook.skills.stealth#stealth>`\ ). 
...
.. list-table::
   ...
   * - Crowd Size
     - Bonus on Perception and Sense Motive Checks to Spot Cheating
   * - Small crowd
     - +8
   * - Medium crowd
     - +12
...
.. _`ultimatecombat.combat.performancecombat#outnumbered`:
...
.. _`ultimatecombat.combat.performancecombat#unfair_advantage`:
...
.. _`ultimatecombat.combat.performancecombat#bribery_or_coercion`:

\ *Bribery or Coercion*\ : Crowds can be bribed or coerced. Typically a successful bribe or attempt at coercion involves paying some amount of gold based on the crowd size, and succeeding at a :ref:`Diplomacy <corerulebook.skills.diplomacy#diplomacy>`\  check with a DC based on the size of the crowd. If the gold is paid and the :ref:`Diplomacy <corerulebook.skills.diplomacy#diplomacy>`\  check fails, the side gains a penalty for cheating instead, though the gold is still spent. If the check succeeds, the DC to improve crowd reaction and gain victory points decreases by 2. For every 5 points over the DC by which the combatants make the check, the DC to improve the crowd's attitude or to gain a victory point decreases by 1. 
...
.. _`ultimatecombat.combat.performancecombat#affecting_the_crowds_attitude`:
...
When a combatant performs or causes one of the triggers described in the sections that follow, he can typically make a performance combat check as a swift action (or a free action if the creature has the :ref:`Master Combat Performer <ultimatecombat.ultimatecombatfeats#master_combat_performer_(combat)>`\  feat) to improve the attitude of the crowd. The swift action usually involves some form of flourish, display, or show in the attempt to grab the attention of the crowd. If the combatant has a performance feat, the action that triggers the check may involve some form of movement or effect. Sometimes a performance combat check can be made as a free or an immediate action. Making a performance check as a free action does not allow a character to take any special action granted to him by a performance feat (unless he has the :ref:`Master Combat Performer <ultimatecombat.ultimatecombatfeats#master_combat_performer_(combat)>`\  feat), and making a performance combat check as an immediate action or as no action never allows a character to perform a special action granted by a performance feat (even if he does have the :ref:`Master Combat Performer <ultimatecombat.ultimatecombatfeats#master_combat_performer_(combat)>`\  feat). 
...
.. _`ultimatecombat.combat.performancecombat#swift_action_checks`:
...
.. _`ultimatecombat.combat.performancecombat#dealing_maximum_damage`:
...
.. _`ultimatecombat.combat.performancecombat#energy_spells_and_effects`:

\ **Energy Spells and Effects**\ : Crowds tend to respond to flashy spells and effects. If a combatant casts a spell or produces an effect that deals acid, cold, fire, electricity, force, or sonic damage in a visible way (including weapons with special abilities like :ref:`flaming burst <corerulebook.magicitems.weapons#weapons_flaming_burst>`\  or :ref:`shocking burst <corerulebook.magicitems.weapons#weapons_shocking_burst>`\  that deal bursts of energy damage on critical hits), she can make a performance combat check as a swift action.

.. _`ultimatecombat.combat.performancecombat#feint`:

\ **Feint**\ : If a combatant successfully feints against an enemy, she can make a performance combat check as a swift action.

.. _`ultimatecombat.combat.performancecombat#knocking_opponent_prone`:
...
.. _`ultimatecombat.combat.performancecombat#multiple_hits`:
...
.. _`ultimatecombat.combat.performancecombat#free_action_or_immediate_action_checks`:
...
.. _`ultimatecombat.combat.performancecombat#critical_hit`:
...
.. _`ultimatecombat.combat.performancecombat#first_blood:`:
...
.. _`ultimatecombat.combat.performancecombat#raging`:
...
.. _`ultimatecombat.combat.performancecombat#vanquish_opponent`:
...
.. _`ultimatecombat.combat.performancecombat#mandatory_checks`:
...
.. _`ultimatecombat.combat.performancecombat#magical_healing`:

\ **Magical Healing**\ : Crowds tend to dislike the use of magical healing; some crowds even see it as cheating. On a turn when a combatant casts a healing spell or other healing effect, or uses a healing spell trigger, spell completion, or use-activated item (including forcing a potion of :ref:`cure light wounds <corerulebook.spells.curelightwounds#cure_light_wounds>`\  or similar potion down the throat of an opponent) she must make a performance combat check. Success yields no change to the attitude of the crowd, but any failure reduces the crowd's attitude by one step.

.. _`ultimatecombat.combat.performancecombat#rolling_a_natural_1`:
...
.. _`ultimatecombat.combat.performancecombat#withdraw`:
...
.. _`ultimatecombat.combat.performancecombat#victory_points`:
...
.. _`ultimatecombat.combat.performancecombat#splitting_up_victory_points`:
...
.. _`ultimatecombat.combat.performancecombat#determining_a_winner`:
...
.. _`ultimatecombat.combat.performancecombat#serialized_performance_combats`:
