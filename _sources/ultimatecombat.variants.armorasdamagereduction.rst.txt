...
.. _`ultimatecombat.variants.armorasdamagereduction`:

.. contents:: \ 

.. _`ultimatecombat.variants.armorasdamagereduction#armor_as_damage_reduction`:
...
The following is an alternative system that allows armor to absorb the damage of attacks, rather than a system that provides an abstract way of determining when a hit does actual harm. Like all the rules presented in this chapter, it is a variant to the base system of the PRD. It is for GMs who want armor to reduce damage rather than increase Armor Class, and replaces the normal rules for armor.
...
.. _`ultimatecombat.variants.armorasdamagereduction#defense`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#losing_dexterity_bonus_to_defense`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#flat_footed_defense`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#defense_and_using_a_shield_without_proficiency`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#using_armor_without_proficiency`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#dr_armor`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#natural_armor_bonus_and_dr`:
...
For instance, if a creature wearing a \ *+2 chain shirt*\  has DR 6/armor is then subject to a :ref:`barkskin <corerulebook.spells.barkskin#barkskin>`\  spell cast by a 6th-level druid (gaining a +3 natural armor bonus), its DR becomes DR 9/armor for the duration of the spell. This DR is bypassed by adamantine, or the attacks of Huge or larger creatures. 
...
.. _`ultimatecombat.variants.armorasdamagereduction#unusual_creatures_and_armor_dr`:

\ **Unusual Creatures and Armor DR**\ : Amorphous or incorporeal creatures have an easier time bypassing DR/armor. When they attack a creature with DR/armor, they typically treat that DR as a fraction of the DR/armor. Attacks by incorporeal creatures entirely ignore the DR unless that DR comes from a force effect or from armor with the :ref:`ghost touch <corerulebook.magicitems.weapons#weapons_ghost_touch>`\  special armor quality. These traits are detailed in Table 5–3.

.. _`ultimatecombat.variants.armorasdamagereduction#critical_hits_and_defense`:
...
.. _`ultimatecombat.variants.armorasdamagereduction#fortification_special_armor_quality`:

\ **Fortification Special Armor Quality**\ : The :ref:`fortification <corerulebook.magicitems.armor#armor_fortification>`\  special armor quality acts in concert with the check, coming into play if the armor check fails. 

.. _`ultimatecombat.variants.armorasdamagereduction#table_armor_composition_and_dr`:

.. list-table:: Table: Armor Composition and DR
   :header-rows: 1
   ...
   * - Material
     - Dr Provided
     - Attacker Size to Bypass DR
   * - Nonmagical armor\ :sup:`1`
     - DR/magic
     - Large 
   * - Magical armor\ :sup:`1`
     - DR/adamantine
     - Huge
   * - Adamantine armor\ :sup:`1`
     - DR/— 
     - Gargantuan

**Notes:**

* \ :sup:`1`\ Brilliant energy weapons bypass any DR provided by armor. In addition, DR from armor is halved if a creature is grappled or entangled, and does not apply when a creature is helpless or pinned.

.. list-table:: Table: Natural Armor Conversion to DR
   :header-rows: 1
   :class: contrast-reading-table
   :widths: auto
   ...
   * - Natural armor with
     - DR/—
     - Gargantuan DR/adamantine
   * - Natural armor with DR/—
     - DR/—\ :sup:`1`
     - Colossal
   * - Natural armor with DR/epic
     - DR/epic\ :sup:`1`
     - n/a

**Notes:**

* \ :sup:`1`\ A creature that already has DR/— or DR/epic increases its DR by an amount equal to its natural armor bonus.

.. _`ultimatecombat.variants.armorasdamagereduction#table_unusual_creatures_and_armor_dr`:

.. list-table:: Table: Unusual Creatures and Armor DR
   :header-rows: 1
   :class: contrast-reading-table
   :widths: auto

   * - Attacking Creature Type
     - DR Modifier
   * - Amorphous\ :sup:`1`
     - ×1/2
   * - Incorporeal
     - ×0 (×1 if DR is from a force effect or :ref:`ghost touch <corerulebook.magicitems.weapons#weapons_ghost_touch>`\  armor)
   * - Swarm of Tiny creatures
     - ×1/2 
   * - Swarm of Diminutive creatures
     - ×1/4 
   * - Swarm of Fine creatures
     - ×0

**Notes:**

* \ :sup:`1`\ Elementals, oozes, oozelike creatures, and any creature with the amorphous defensive ability.
